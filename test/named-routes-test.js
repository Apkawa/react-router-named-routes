...
var createComplexRouteTree = function (slash) {
  var append = ''
  if (slash) {
    append = '/'
  }
  return (
    React.createElement(Route, {component: Component, name: 'root', path: '/' },
      [
        React.createElement(Route, {component: Component, path: '/app' + append},
          [
            React.createElement(IndexRoute, {name: 'app.index'}),
            React.createElement(Route, {name: 'app.list'})
          ]),

        React.createElement(Route, {name: 'users', path: '/users' + append},
          [
            React.createElement(IndexRoute, {name: 'users.index'}),
            React.createElement(Route, {name: 'users.list', path: 'list' + append}),
            React.createElement(Route, {name: 'users.show', path: ':id' + append}),
            React.createElement(Route, {path: ':id/edit' + append})
          ])
      ])
  )
}
...
describe('NamedURLResolver', function () {
  ...
  it('correctly maps route tree with end slashes', function () {
    resolver = new NamedURLResolverClass()
    resolver.mergeRouteTree(
      React.createElement(Route, {}, [
        React.createElement(Route, {name: 'users.show', path: '/users/:id/'}),
  ...
  it('correctly resolve named routes with end slashes', function () {
    resolver.mergeRouteTree(createComplexRouteTree(true))
    expect(resolver.resolve('root')).to.equal('/')
    expect(resolver.resolve('app.index')).to.equal('/app/')
    expect(resolver.resolve('app.list')).to.equal('/app/')
  ...
  it('correctly formats named routes', function () {
    ...
    expect(resolver.resolve('users.show', {id: 'id/:id'}))
      .to
      .equal('/users/' + encodeURIComponent('id/:id'))
    expect(resolver.resolve('test1', {id: 'id/:id', 'id-parent': 'idp:id'}))
      .to
      .equal('/users/' + encodeURIComponent('idp:id') + '/' + encodeURIComponent('id/:id'))
    expect(resolver.resolve('test2', {colon: 'colon:colon'}))
      .to
      .equal('/users/semi:colon/' + encodeURIComponent('colon:colon'))
  })
  ...
  it('correctly resolves optional params', function () {
    ...
    expect(resolver.resolve('test8', {param1: 'p(1)', param2: 'p(2)'}))
      .to
      .equal('/path/p(1)/p(2)', 'Params with parentheses')
    ...
    expect(resolver.resolve('test9', {param1: 1, param2: 2}))
      .to
      .equal('/path/1/2/', 'Resolved sequence')
  })
  ...
  it('correctly resolves splat params', function () {
    ...
    expect(resolver.resolve('test1', {splat: 'slash/here'}))
      .to
      .equal('/some/' + encodeURIComponent('slash/here'), 'Slash with single star')
    ...
    expect(resolver.resolve('test3', {splat: 'slash/here'}))
      .to
      .equal('/some/slash/here', 'Slash with double star')

    expect(resolver.resolve('test5', {splat: 'first/splat'}))
      .to
      .equal('/some/' + encodeURIComponent('first/splat') + '/path/', 'Multiple globs 1')
    expect(resolver.resolve('test5', {splat: ['first/splat', 'second/splat']}))
      .to
      .equal('/some/' + encodeURIComponent('first/splat') + '/path/second/splat',
        'Multiple globs 2')
...
describe('Link', function () {
  ...
  it('correctly renders <Link /> elements', function () {
    ...
    expect(root.children[i++].getAttribute('href'))
      .to
      .equal('/users/' + encodeURIComponent(':mal/ici/:ous'))
